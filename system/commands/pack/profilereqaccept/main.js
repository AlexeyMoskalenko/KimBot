...
function(arg, _, aliascommand){
    ...
    collectionlist.findOne({hash: aliascommand.commandargs[0]}, (err, document) =>{
        if (err){
            let errmsg = Dictionary.errors.mongodberror.replace("#00", "#20"); 
        ...
        if (document != null){
            ...
            if (role === undefined) return arg.msg.reply(Dictionary.errors.profilereqaccrolenotfound);

            collectionlist.deleteOne(document, (_err, _res) => {
                
                if (err){
                    let errmsg = Dictionary.errors.mongodberror.replace("#00", "#21"); 
                    return arg.msg.reply(errmsg);
                }
                
                if (_res.result.ok != 1){
                    let errmsg = Dictionary.errors.mongodberror.replace("#00", "#23"); 
                    return arg.msg.reply(errmsg);
                }
                ...
                if (user){
                    ...
                    try{
                        ...
                        let replymsg = Dictionary.reply.profilereqacc.replace("#hash", aliascommand.commandargs[0]);
